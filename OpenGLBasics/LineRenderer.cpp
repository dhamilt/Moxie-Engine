...
void LineRenderer::Draw(mat4 projection, mat4 view)
{
	...
	shader->SetFloat4("lineColor", lineColor.r, lineColor.g, lineColor.b, lineColor.a);
	shader->SetBool("_lerp", lerpColor);
	if (lerpColor)
	{
		shader->SetFloat4("startColor", startColor.r, startColor.g, startColor.b, startColor.a);
		shader->SetFloat4("endColor", endColor.r, endColor.g, endColor.b, endColor.a);
	}
...
void LineRenderer::LerpColors(Color _start, Color _end)
{
	lerpColor = true;
	startColor = _start;
	endColor = _end;
...
void LineRenderer::SetLineColor(const Color& color)
{
	lineColor = color;
}
...
void LineRenderer::UpdateUVs()
{
	...
	for (int i = 0; i < linePoints.size(); i++)
	{
		...
		// beginning = 0; end = 1
		info.u = MathLibrary<float>::Lerp(0.0f, 1.0f, (float)i / (float)(linePoints.size() - 1));
