...
bool LoadModelFromFile(const char* path, std::vector<vertex>& vertices, std::vector<uint32_t>&indices);
//void ProcessNode(aiNode* node, const aiScene scene);
void ProcessMesh(const aiScene*, std::vector<vertex>&vertices, std::vector<uint32_t>&indices);
GLuint LoadBMP(const char* imgPath);
bool LoadObjC(const char* filePath, std::vector<vertex>& vertices, std::vector<GLuint>& indices);
...
{	
	...
	MSoundDataAsset* sound = new MSoundDataAsset("../Aritus - Summer With You.wav");
	MSoundDataAsset* sound2 = new MSoundDataAsset("../Large Professor - Frantic Barz (NIKK BLVKK Remix).wav");
	...
	bool modelLoaded = LoadObj("cube.obj", vertices, indices);
	if (!modelLoaded)
		return 0;
	...
	monkeyMeshComponent->mesh->SetMateialColor(MLinearColor(20.0f, 20.0f, 0.0f, 1.0f));
	...
	// Make a icosahedron mesh
	//MeshComponent* icosahedronMeshComponent = new MeshComponent();
	//// Get icosahedron vertices
	//std::vector<TriangleIndices> icoFaces;
	//std::vector<vector3> icoVertices;
	//std::vector<uint32_t> icoIndices;
	//std::vector<vector3> icoNormals;
	//PrimitivesLibrary::GetIcosahedron(1.0f, 1, icoVertices, icoFaces,  icoIndices);
	//PrimitivesLibrary::ComputeNormals(icoIndices, icoVertices, icoNormals);
	//icosahedronMeshComponent->mesh = new Mesh(icoVertices, icoNormals, icoIndices, "Lighting.vertex", "Lighting.fragment");
	//icosahedronMeshComponent->mesh->ApplyTexture(texture);
	//icosahedronMeshComponent->transform.Translate(vector3(3, 1, 7));
	//actor->AddComponent(icosahedronMeshComponent);
	// Make a light 
	MyLightComponent* lightComponent = new MyLightComponent();		
	lightComponent->transform.Rotate(vector3(0.0f, 1.0f, 0.0f), 90);
	lightComponent->transform.Translate(vector3(-5, 1, 7));
	lightComponent->lightColor = MLinearColor(10.0f, 5.0f, 7.5f, 1.0f);
	lightComponent->radius = 0.25f;
	...
	lineEQ->GetSource()->SetVolume(.15f);
	// Change the color of the component
	MLinearColor cyan = Cyan;
	MLinearColor green = Green;
	lineEQ->AddLerpColors(cyan * 5, green * 30.0f);
...
void ProcessMesh(const aiScene* scene, std::vector<vertex>& vertices, std::vector<uint32_t>& indices)
{
	if (scene)
	{
